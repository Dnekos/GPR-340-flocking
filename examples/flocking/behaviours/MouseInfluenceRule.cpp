...
Vector2 MouseInfluenceRule::computeForce(const std::vector<Boid*>& neighborhood, Boid* boid) {
    ...
    if (ImGui::IsMousePosValid() && io.MouseDown[0]) {
        ...
        Vector2 displacement = mousePos - boid->getPosition();//Vector2::zero(); // todo: change this
        float distance = Vector2::getDistance(boid->getPosition(), mousePos);// 0; // todo: change this
